...
checkWall:
    ...
    la $t5, wallColor   # Load address of wall color
    lw $t5, 0($t5)      # Load wall color value into $t5
    lw $t6, 0($t4)      # Load the color of the new position into $t6
    beq $t5, $t6, wallDetected  # If the new position is a wall, jump to wallDetected
    jr $ra               # Otherwise, return to the caller

wallDetected:
    # Restore the player to their original position (stored in $t0, $t1)
    mul $t4, $t0, $t7    # Calculate the memory address for the old position (row index * columns)
    add $t4, $t4, $t1    # Add column index
    mul $t4, $t4, 4      # Multiply by 4 (word size)
    add $t4, $t4, $gp    # Add the global pointer

    # Restore the old position to the passage color
    la $t5, passageColor  # Load passage color
    lw $t5, 0($t5)        # Load passage color value
    sw $t5, 0($t4)        # Restore old position to passage color

    # Return to the game loop without moving the player
    jr $ra                
exit:
